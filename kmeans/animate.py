...
axs[1].set_ylim(k.inertia_log[k.n_iter_ - 1] - 0.05*k.inertia_log[0], 1.05*k.inertia_log[0])
...
for index in range(k.n_clusters):
	scatters.append(axs[0].scatter(k.X[:, 0], k.X[:, 1], alpha = .2, s = 15, color = colors[index]))
	scatters.append(axs[0].scatter([],[], alpha = .8, s = 45, color = colors[index]))
...
def show_animation(frame):
	...
	axs[1].plot(range(1, frame + 2), k.inertia_log[:frame + 1], color = colors[0])


anim = FuncAnimation(fig, show_animation, interval = 200, frames = range(k.n_iter_))
